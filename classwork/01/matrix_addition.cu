...
__global__ void matrix_addition(float* mat_a, float* mat_b, float* mat_c) {
...
int main(void) {
    ...
    // 执行 GPU 内核函数
    matrix_addition << < blocksPreGrid, threadsPreBlock >> > (device_mat_a, device_mat_b, device_mat_c);
